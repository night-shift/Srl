...
namespace Tools {
    ...
    std::string type_to_str (const Value& value);
    size_t      type_to_str (const Value& value, std::vector<uint8_t>& out);

    std::pair<bool, Value> str_to_type (const String& str_wrap, Type hint = Type::Null);
    std::pair<bool, Value> str_to_type (const uint8_t* str, size_t str_len, Type hint = Type::Null);
    ...
    std::vector<uint8_t> conv_charset (Encoding target_encoding, const char* str, size_t str_len, bool throw_error = true);

    size_t               conv_charset (Encoding target_encoding, const String& str_wrap,
                                       std::vector<uint8_t>& buffer, bool throw_error, size_t buffer_index = 0);
    template<class TChar>
    std::basic_string<TChar> conv_charset (const String& string_wrap, bool throw_error);
